...
namespace fsm
{
    public class FSMStateController : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            for (int i = _statesSerialized.Count - 1; i >= 0; i--)
            {
                ...
                _statesCached[serializedState.EventType].RegisterAllowedAllEvents(serializedState.AllowAllevents);
                _statesCached[serializedState.EventType].RegisterAllowedEvents(serializedState.AllowedEventTypes);
                _statesCached[serializedState.EventType].RegisterIgnoredEvents(serializedState.IgnoredEventTypes);
        ...
        public void ReceiveGameEvent(FSMEventType eventType, object data)
        {
            ...
            if (!_currentState.IsEventAllowed(eventType)) return;
    ...
    internal class SerializableState
    {
        ...
        [LabelText("Allow All Events")]
        [Tooltip("Enabling this will allow all events")]
        [SerializeField]
        public bool AllowAllevents = true;

        [LabelText("Allowed Events")]
        [ShowIf("@this.AllowAllevents == false")]
        ...
        [LabelText("Ignored Events")]
        [ShowIf("@this.AllowAllevents == true")]
        [ListDrawerSettings(Expanded = false, ShowIndexLabels = false, ShowItemCount = false, DraggableItems = true, HideRemoveButton = false, HideAddButton = false)]
        [SerializeField]
        public List<FSMEventType> IgnoredEventTypes = new List<FSMEventType>();
